...
Two practical shortcuts: `/dev/stories` is reachable **without auth** (it sits outside `DashboardLayout`), so shared components can be verified there at any viewport without a wallet — add new primitives to that gallery as part of building them. The dashboard screens sit behind the `signMessage` gate, which automation cannot cross; verify their changed components standalone on `/dev/stories` and flag the wallet-gated click-through for the user.
