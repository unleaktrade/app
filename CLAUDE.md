...
- **liquidity-guard sets no CORS headers**. In dev we route through the Vite proxy at `/liquidity-guard/*` (configured in `vite.config.ts`). All client code must call `/liquidity-guard/health` and `/liquidity-guard/check`, never `env.liquidityGuardUrl` directly. Production deployment needs same-origin hosting or a CORS-enabled reverse proxy — open follow-up after #11/#26.
